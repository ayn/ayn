# Andrew Ng

[![ReadMeSupportPalestine](https://raw.githubusercontent.com/Safouene1/support-palestine-banner/master/banner-support.svg)](https://pcrf1.app.neoncrm.com/forms/gaza-recovery)

I build production-ready MVPs in 4-6 weeks for founders who want to launch fast without hiring a full team. Engagements typically land in the $50k-$100k range. More at [andrewng.com](https://andrewng.com).

Curator at [Lightmist Coffee](https://lightmist.coffee) in Taipei.

I work across coffee, software, and small internet tools.

## Current Interests

- Building things for Lightmist Coffee and related publishing workflows
- Homelab projects, self-hosting, and practical home infrastructure
- Home Assistant, smart home automations, sensors, and air quality
- DNS, networking, and small ops-friendly stacks
- Small utilities that solve a specific problem well

## Elsewhere

- [Website](https://andrewng.com)
- [Blog](https://blog.andrewng.com)
- [Strava](https://strava.com/athletes/ayn)
- [Mastodon](https://trunk.lol/@ayn)
- [Instagram](https://instagram.com/ayn)
- [LinkedIn](https://www.linkedin.com/in/andrewng)
